...
public class LogThread implements Callable<Boolean> {
    ...
    public Boolean call() throws Exception {
        ...
        while (DOWNLOAD_FINISH_THREAD.get() != DownloadMain.DOWNLOAD_THREAD_NUM) {
            double downloadedSize = DOWNLOAD_SIZE.get();
            downSizeArr[++i % 5] = Double.valueOf(downloadedSize - size).intValue();
            size = downloadedSize;
            ...
            // 剩余时间
            double surplusSize = httpFileContentLength - downloadedSize - LOCAL_FINISH_SIZE.get();
            ...
            // 已下载大小
            String currentFileSize = String.format("%.2f", downloadedSize / mb + LOCAL_FINISH_SIZE.get() / mb);
